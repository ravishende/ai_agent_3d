...
def INIT_GAME(
        game_map: list[int] | list[np.ndarray],
        map_width: int | None = None,
        trap_cols: list[int] | None = None,
        trap_death_prob: float = 0.3
    ) -> None:
    """
    Initialize the game board, start location, trap columns, and trap probability.
    The game board is initialized as a list of bitmaps, 
    regardless of if the game_map is a list of bitmaps or numpy arrays.
    Parameters:
        game_map: list of all obstacle slices in the game (as bitmaps or numpy arrays)
        map_width: number of columns in a map grid
    ...
    """
    death_trap_prob_msg = "trap_death_prob must be at least 0 and less than 1 -- trap_prob in [0,1)"
...
def GRID_AT(t: int, as_bitmap=True) -> int | np.ndarray | None:
    """
    Get the grid at time index t or None if past the end of the game map.
    Use as_bitmap=False to return an array instead of a bitmap (int)"""
    assert _GAME_MAP is not None, "Game map has not been initialized. Call INIT_GAME first."
    if t < 0 or t >= len(_GAME_MAP):
        return None
    if as_bitmap:
        return _GAME_MAP[t]
    return bitmap_to_array(_GAME_MAP[t], map_width=_MAP_WIDTH, n_rows=_N_ROWS)
...
def VIEW_GRIDS(t: int, n_grids:int = 2, as_bitmap=False) -> tuple[list[np.ndarray], list[int]]:
    """View up to n future grids and their n trap cols starting from time index t."""
    grids = []
    trap_cols = []
    for offset in range(n_grids):
        grid = GRID_AT(t+offset, as_bitmap=as_bitmap)
...
class State:
    ...
    def __init__(self, time_index:int, player_col:int):
        ...
        self.grid_bitmap: int | None = GRID_AT(time_index)
    ...
    def is_terminal(self) -> bool:
        ...
        return self.grid_bitmap is None
    ...
    def get_reward(self, action: Action) -> int:
        ...
        """
        assert self.grid_bitmap is not None, "Cannnot perform more actions from a terminal state."

        new_location = self._update_location(self.player_col, action)
        # action results in immediate death --> no reward
        if self._collides(new_location, self.grid_bitmap):
            return 0
        # no death --> reward = 1
        return 1
    ...
    def __str__(self):
        prefix_s = "\nLocation: col=%i\nGrid Bitmap: %i\nGrid (t=%i):\n"
        prefix = prefix_s.format(self.player_col, self.grid_bitmap, self.time_index)
        grid = "None"
        if self.grid_bitmap is not None:
            grid = bitmap_to_array(self.grid_bitmap, map_width=_MAP_WIDTH, n_rows=_N_ROWS)
        suffix = str(grid) + "\n" + "="*50
