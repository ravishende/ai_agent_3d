...
class IntegratedPolicyIteration:
    ...
    def policy_evaluation(self, theta=1e-6, max_iterations=1000):
        ...
        while iteration < max_iterations:
            ...
            value_changes.append(np.mean(iteration_changes))
            iteration += 1
            if delta < theta:
                break
    ...
    def run(self, max_iterations=100, eval_theta=1e-6, verbose=True):
        for iteration in range(max_iterations):
            ...
            self.metrics['max_value_change'].append(max_value_change)
            self.metrics['mean_value_change'].append(mean_value_change)
            self.metrics['policy_changes'].append(num_changes)
            self.metrics['eval_iterations'].append(eval_iters)
